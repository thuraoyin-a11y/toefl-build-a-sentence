...
### BUG-003 — Login Hardcoded Demo IDs Causing Practice Set 404
- **Title:** Student practice page showed "Practice set not found" after login
- **Symptom:** Students could log in but got 404 when accessing assigned practice sets
- **Root Cause:** `app/api/auth/login/route.ts` had hardcoded demo `studentId`/`teacherId` (e.g., `student-alex-001`) that never matched the UUIDs generated by `prisma/seed.ts`. The session stored a fake ID, so `/api/practice-sets/[id]` could not find any assignment for that student.
- **Fix:** Removed the hardcoded demo account branch in login route; all logins now go through `authenticateUser()`, which already supports demo accounts and returns real database UUIDs.
