...
import type { AttemptResponse, StudentInfo } from "@/lib/teacher/types";
...
export default function StudentAttemptsPage() {
  ...
  useEffect(() => {
    // Fetch student info and attempts in parallel
    Promise.all([
      fetch(`/api/teacher/students/${studentId}`).then((res) => {
        if (!res.ok) throw new Error("Student not found");
        return res.json();
      }),
      fetch(`/api/teacher/students/${studentId}/attempts`).then((res) => {
        if (!res.ok) throw new Error("Failed to fetch attempts");
        return res.json();
      }),
    ])
      .then(([studentData, attemptsData]) => {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-apple-text-secondary">Attempt History</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Target className="h-5 w-5 text-apple-text-secondary" />
